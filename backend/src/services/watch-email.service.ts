...
const getAllBankEmails = async (
  userId: string,
  userInfo: { currentUser: User; access_token: string },
) => {
...
const addBankEmail = async (
  ...
  if (email === currentUser.email)
    throw new Error('cannot set watch email same as user email');
  ...
  const res = await dbSource.manager.transaction(
